...
- **Category / Topic:** Entertainment, Self-development, Small commercial part
...
- Bottom tabs: browse informational sections and Cart screen
- Shop screen: browse tarot decks and books, add/remove items from cart, adjust quantity, proceed to cart and checkout
- Login/Register screens: user auth state updates
...
- **Profile Screen:** ⚙️ (use the gear icon in the header) edit avatar (Image Picker), add address & phone, switch Dark/Light theme, change password, logout
...
- On Login/Register [→] (use the login icon next to the gear icon in header) → user auth state updates
...
_UI updates automatically via Firebase listeners (real-time updates)_
